...
def extract_windows_from_sequence(seq_id, seq: Seq, window_size: int, step_size: int):
    seq_len = len(seq)
    windows = []
    u_id = 0
    ...
    df = pd.DataFrame(windows, columns=['u_id', 'seq_id', 'start', 'end', 'seq', 'strand'])
    
    return df

def extract_sliding_windows(ref_genome_dict: dict, window_size: int,
...
def main():
    ...
    # Convert windows to a Pandas DataFrame
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            df_slide.to_csv(os.path.join(temp_dir, sliding_windows_filename),
                            index=False)
            ...
            sample_fasta_path = os.path.join(temp_dir, SAMPLE_FASTA)
            df_to_fasta(df_slide.reset_index(drop=True), sample_fasta_path, "training")

            # remove output folder if it exists
            output_sample_dir = os.path.join(temp_dir, OUTPUT_SAMPLE)
            ...
            data_path = os.path.join(temp_dir, 'output_sample/')
            print('\nLoading data')
            ...
            for embedding in input_dim_dict.keys():
                ...
                out_embed.to_csv(os.path.join(temp_dir, embed_filename), index=False)
